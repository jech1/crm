          ...
          return (
            <div key={stage} className="flex-shrink-0 w-64">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs text-slate-400 dark:text-slate-500">{stageRestaurants.length}</span>
              </div>
              ...
              <div className="space-y-2">
                {stageRestaurants.map((r) => (
                  <Link
                    ...
                    className="block rounded-lg border bg-white dark:bg-slate-800 dark:border-slate-700 p-3 hover:shadow-sm transition-shadow"
                  >
                    <p className="text-sm font-medium text-slate-900 dark:text-white truncate">{r.name}</p>
                    <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{r.city}</p>
                    <div className="flex items-center justify-between mt-2">
                      <span className="text-xs text-slate-500 dark:text-slate-400">{r._count.visits} visit{r._count.visits !== 1 ? "s" : ""}</span>
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-[10px] text-slate-400 dark:text-slate-500">{r.opportunityScore}</span>
                      </div>
                    </div>
                  </Link>
                ))}
                {stageRestaurants.length === 0 && (
                  <div className="rounded-lg border border-dashed bg-slate-50 dark:bg-slate-800/50 dark:border-slate-700 p-4 text-center">
                    <p className="text-xs text-slate-400 dark:text-slate-500">No restaurants</p>
                  </div>
                )}
              </div>
            </div>
